...
</svg>`,F=`<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
</svg>`,O='<svg class="sv-spinner" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10" stroke-opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" stroke-linecap="round"/></svg>',R=`
/* ── Theme variables ─────────────────────────────── */
:root {
...
`,z="https://api.anthropic.com/v1/messages",H="claude-sonnet-4-20250514",U=`You are a GLSL shader generator. Output ONLY valid WebGL2 GLSL fragment shader code. No explanation, no markdown — just the raw GLSL code inside a single \`\`\`glsl code block.
...
Make shaders that are visually stunning, smooth, and reactive to the audio uniforms. Multiply audio reactivity by u_intensity. Use interesting math — sin, cos, noise, polar coordinates, fractals, etc.`;function G(r){const t=r.match(/```glsl\s*\n([\s\S]*?)```/);if(t)return t[1].trim();const e=r.match(/```\s*\n([\s\S]*?)```/);if(e)return e[1].trim();const s=r.trim();if(s.startsWith("#version"))return s;throw new Error("No valid GLSL code found in LLM response")}async function V(r,t,e,s){let i=`Scene: ${r}
Vibe: ${t}
...
${s}`);const o=await fetch(z,{method:"POST",headers:{"x-api-key":e,"anthropic-version":"2023-06-01","content-type":"application/json","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:H,max_tokens:4096,system:U,messages:[{role:"user",content:i}]})});if(!o.ok){const h=await o.text();throw o.status===401?new Error("Invalid API key. Check your Anthropic API key in settings."):o.status===429?new Error("Rate limited. Wait a moment and try again."):new Error(`API error (${o.status}): ${h}`)}const u=(await o.json()).content?.[0]?.text;if(!u)throw new Error("Empty response from Claude API");return G(u)}async function _(r,t,e,s){return V(r,t,e,s)}const g=[{name:"Cosmic Ocean",description:"Blue/purple fluid simulation — bass drives wave amplitude",source:`#version 300 es
...
`;class D{constructor(){n(this,"audio");n(this,"renderer");n(this,"ui");n(this,"intensity",.5);n(this,"startTime",0);n(this,"running",!1);n(this,"loop",()=>{if(!this.running)return;const t=this.audio.getUniforms(),s={u_time:performance.now()/1e3-this.startTime,u_bass:t.u_bass,u_mid:t.u_mid,u_high:t.u_high,u_beat:t.u_beat,u_intensity:this.intensity,u_resolution:[window.innerWidth,window.innerHeight]};this.renderer.render(s),requestAnimationFrame(this.loop)});this.audio=new C,this.renderer=new L,this.ui=new P}async init(){const t=document.getElementById("canvas");if(!t)throw new Error("Canvas element not found");this.renderer.init(t,w),this.renderer.onError=s=>this.ui.showToast(s,4e3),this.ui.init(),this.wireUI();const e=localStorage.getItem("shader-visuals-intensity");e!==null&&(this.intensity=parseFloat(e)),g.length>0&&this.renderer.crossfadeTo(g[0].source,300),this.startTime=performance.now()/1e3,this.running=!0,this.loop(),this.registerSW()}wireUI(){this.ui.onGenerate=async(t,e)=>{const s=localStorage.getItem("shader-visuals-api-key")||"";if(!s){this.ui.showToast("Set your API key in settings first",3e3);return}if(!t.trim()&&!e.trim()){this.ui.showToast("Enter a scene or vibe first",2e3);return}this.ui.setLoading(!0);try{const i=await _(t,e,s,void 0);if(!this.renderer.crossfadeTo(i)){this.ui.showToast("Shader had errors, retrying...",2e3);try{const l=await _(t,e,s,"Previous shader failed to compile. Please output simpler, valid GLSL.");this.renderer.crossfadeTo(l)||(this.ui.showToast("Shader failed to compile. Using fallback.",3e3),this.renderer.crossfadeTo(w))}catch(l){this.ui.showToast(`Retry failed: ${l.message}`,3e3)}}}catch(i){this.ui.showToast(`Generation failed: ${i.message}`,4e3)}finally{this.ui.setLoading(!1)}},this.ui.onPresetSelect=t=>{const e=g.find(s=>s.name===t);e&&this.renderer.crossfadeTo(e.source)},this.ui.onIntensityChange=t=>{this.intensity=t,localStorage.setItem("shader-visuals-intensity",String(t))},this.ui.onMicToggle=async()=>{if(this.audio.isActive&&this.audio.currentMode==="mic")this.audio.stop(),this.ui.setMicActive(!1),this.ui.setMusicActive(!1);else try{await this.audio.initMic(),this.ui.setMicActive(!0),this.ui.setMusicActive(!1)}catch{this.ui.showToast("Microphone access denied",3e3)}},this.ui.onMusicFile=t=>{try{this.audio.initFile(t),this.ui.setMusicActive(!0),this.ui.setMicActive(!1),this.ui.showToast(`Playing: ${t.name}`,2e3)}catch{this.ui.showToast("Failed to play audio file",3e3)}},this.ui.onApiKeyChange=t=>{localStorage.setItem("shader-visuals-api-key",t)}}async registerSW(){if("serviceWorker"in navigator)try{await navigator.serviceWorker.register("/shader-visuals/"+"sw.js")}catch{}}}const N=new D;N.init().catch(r=>{console.error("Failed to initialize app:",r),document.body.innerHTML=`
